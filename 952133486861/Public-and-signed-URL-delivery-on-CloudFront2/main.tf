terraform {
  ...
  backend "s3" {
    ...
    key     = "952133486861/Public-and-signed-URL-delivery-on-CloudFront2/main.tfstate"
    region  = "us-west-2"
    encrypt = true
  }
}
...
resource "aws_cloudfront_distribution" "cdn-aberta" {
  ...
  origin {
    ...
    origin_access_control_id = aws_cloudfront_origin_access_control.oac_origem-cf-validacao.id
...
resource "aws_cloudfront_distribution" "cdn-protegida" {
  ...
  default_cache_behavior {
    cache_policy_id           = data.aws_cloudfront_cache_policy.policy_cachingoptimized.id
    field_level_encryption_id = aws_cloudfront_field_level_encryption_config.cifra-cartao.id
...
resource "aws_cloudfront_field_level_encryption_config" "cifra-cartao" {
  content_type_profile_config {
    forward_when_content_type_is_unknown = true
    content_type_profiles {
      items {
        profile_id   = aws_cloudfront_field_level_encryption_profile.perfil-cartao.id
...
resource "aws_cloudfront_field_level_encryption_profile" "perfil-cartao" {
  name = "perfil-cartao"
  encryption_entities {
    items {
      provider_id   = "cloudman-fle-provider"
      public_key_id = aws_cloudfront_public_key.chave-assinatura.id
      field_patterns {
        items = ["credit-card-*"]
      }
    }
...
resource "aws_cloudfront_origin_access_control" "oac_origem-cf-validacao" {
...
resource "aws_cloudfrontkeyvaluestore_key" "banner1" {
  key                 = "banner"
  key_value_store_arn = aws_cloudfront_key_value_store.kvs-validacao.arn
...
data "aws_iam_policy_document" "aws_s3_bucket_policy_origem-cf-validacao_st_Public-and-signed-URL-delivery-on-CloudFront2_doc" {
  statement {
    ...
    resources = ["${aws_s3_bucket.origem-cf-validacao.arn}/*"]
    condition {
      test     = "StringEquals"
      variable = "AWS:SourceArn"
      values   = ["arn:aws:cloudfront::${data.aws_caller_identity.current.account_id}:distribution/${aws_cloudfront_distribution.cdn-aberta.id}"]
    }
  }
  statement {
    ...
    condition {
      test     = "StringEquals"
      variable = "AWS:SourceArn"
      values   = ["arn:aws:cloudfront::${data.aws_caller_identity.current.account_id}:distribution/${aws_cloudfront_distribution.cdn-protegida.id}"]
    }
  }
}

resource "aws_s3_bucket_policy" "aws_s3_bucket_policy_origem-cf-validacao_st_Public-and-signed-URL-delivery-on-CloudFront2" {
  bucket = aws_s3_bucket.origem-cf-validacao.id
  policy = data.aws_iam_policy_document.aws_s3_bucket_policy_origem-cf-validacao_st_Public-and-signed-URL-delivery-on-CloudFront2_doc.json
}
...
resource "aws_s3_object" "index-html2" {
  acl          = "private"
  bucket       = aws_s3_bucket.origem-cf-validacao.bucket
  content      = "<!doctype html><meta charset=\"utf-8\"><title>Struct8 CloudFront</title><h1>CloudFront servindo do S3</h1><p>Se voce esta lendo isto pelo dominio do CloudFront, o Origin Access Control, a distribution e a policy de cache subiram e funcionam.</p>"
